...
export const menuService = {
  getAllMenuItems: async (page = 0, size = 20) => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/menu/items?page=${page}&size=${size}`, {
        method: 'GET',
        headers: {
  ...
  getAllVariations: async (page = 0, size = 20) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error('Authentication required');
      ...
      const response = await fetch(`${API_URL}/api/menu/variations?page=${page}&size=${size}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
